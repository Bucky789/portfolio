...
const Logo = () => (
  <div className="flex h-10 w-10 items-center justify-center rounded-full border border-white/30 text-sm font-bold text-white">
    MS
  </div>
);
...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-6 left-1/2 z-50 -translate-x-1/2">
      <div
        className=" relative flex items-center gap-6 rounded-full border border-white/20 bg-[#00ADB5]/25 px-4 py-2 backdrop-blur-xl shadow-lg"
      >
        {/* Logo */}
        <Logo />
        ...
        <button
          ...
          className=" md:hidden flex h-9 w-9 items-center justify-center rounded-full border border-white/30 text-white "
...
{menuOpen && (
  <div className="md:hidden absolute top-full left-0 w-full mt-2">
    <div
      className=" rounded-2xl border border-white/20 bg-[#00ADB5] shadow-lg px-4 py-4 flex flex-col gap-3 text-base font-medium "
    >
      {["Projects", "Skills", "Contact"].map((item) => (
        <Link
          key={item}
